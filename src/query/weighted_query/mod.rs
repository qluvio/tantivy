...
pub use self::weighted_scorer::WeightedScorer;
pub use self::weighted_weight::WeightedWeight;
...
mod tests {
    ...
    use crate::query::{Query, QueryParser, Scorer, TermQuery, WeightedQuery};
    ...
    #[test]
    pub fn test_weighted_term_query_no_freq() {
        ...
        let query_score = Score::new((1, 2.));
        let weighted_term_query = WeightedQuery::new(term_query, query_score);
        let term_weight = weighted_term_query.weight(&searcher, true).unwrap();
        let segment_reader = searcher.segment_reader(0);
        let mut term_scorer = term_weight.scorer(segment_reader).unwrap();
        assert!(term_scorer.advance());
        assert_eq!(term_scorer.doc(), 0);
        assert_eq!(term_scorer.score(), query_score + Score::new(0.28768212));
    }

    #[test]
    pub fn test_weighted_term_weight() {
        let mut schema_builder = Schema::builder();
        ...
        let searcher = index.reader().unwrap().searcher();
        {
            ...
            let topdocs = searcher
                .search(&weighted_term_query, &TopDocs::with_limit(2))
                .unwrap();
            assert_eq!(topdocs.len(), 1);
            let (score, _) = topdocs[0];
            assert_eq!(query_score.0, score.0);
            assert_nearly_equals(query_score.1 + 0.77802235, score.1);
        }
        {
            ...
            assert_eq!(query_score.0, score1.0);
            assert_nearly_equals(query_score.1 + 0.27101856, score1.1);
            let (score2, _) = top_docs[1];
            assert_eq!(query_score.0, score2.0);
            assert_nearly_equals(query_score.1 + 0.13736556, score2.1);
        }
        {
            let query_parser = QueryParser::for_index(&index, vec![]);
            let query = query_parser
                .parse_query("left#1:left2 left#3:left1")
                .unwrap();
            let top_docs = searcher.search(&query, &TopDocs::with_limit(2)).unwrap();
            assert_eq!(top_docs.len(), 2);
            let (score1, _) = top_docs[0];
            assert_eq!(3, score1.0);
            assert_nearly_equals(0.9153879, score1.1);
            let (score2, _) = top_docs[1];
            assert_eq!(3, score2.0);
            assert_nearly_equals(0.27101856, score2.1);
        }
    }

    #[test]
    fn test_weighted_term_query_count_when_there_are_deletes() {
        let mut schema_builder = Schema::builder();
        ...
        assert_eq!(weighted_term_query.count(&*reader.searcher()).unwrap(), 1);
    }

    #[test]
    fn test_weighted_term_query_debug() {
        let term_query = TermQuery::new(
        ...
        let weighted_term_query = WeightedQuery::new(term_query, query_score);
        assert_eq!(
            format!("{:?}", weighted_term_query),
            "WeightedQuery(TermQuery(Term(field=1,bytes=[104, 101, 108, 108, 111])), Score(1, 2.0))"
        );
    }
}
